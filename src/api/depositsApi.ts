...
import { CreateSenPaymentResponse } from '../model/createSenPaymentResponse';
import { MockSenPaymentRequest } from '../model/mockSenPaymentRequest';
import { NotAuthorized } from '../model/notAuthorized';
import { SearchBusinessAccountDepositsResponse } from '../model/searchBusinessAccountDepositsResponse';
...
export class DepositsApi {
     ...
     * @param mockSenPaymentRequest 
     */
    public async createSenPayment (mockSenPaymentRequest?: MockSenPaymentRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CreateSenPaymentResponse;  }> {
        const localVarPath = this.basePath + '/v1/mocks/payments/sen';
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(mockSenPaymentRequest, "MockSenPaymentRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CreateSenPaymentResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "CreateSenPaymentResponse");
     ...
     * @param type Unique identifier for the deposit type. Filters results to fetch deposits made by this specific type.
     ...
     */
    public async searchBusinessAccountDeposits (type?: 'wire', from?: Date, to?: Date, pageBefore?: string, pageAfter?: string, pageSize?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: SearchBusinessAccountDepositsResponse;  }> {
        const localVarPath = this.basePath + '/v1/businessAccount/deposits';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        // give precedence to 'application/json'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: SearchBusinessAccountDepositsResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "SearchBusinessAccountDepositsResponse");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
