...
import { GetReversalsResponse } from '../model/getReversalsResponse';
...
export enum ReversalsApiApiKeys {
}

export class ReversalsApi {
    ...
    public setApiKey(key: ReversalsApiApiKeys, value: string) {
        (this.authentications as any)[ReversalsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getReversals (status?: 'pending' | 'confirmed' | 'paid' | 'failed' | 'action_required', from?: Date, to?: Date, pageBefore?: string, pageAfter?: string, pageSize?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetReversalsResponse;  }> {
        const localVarPath = this.basePath + '/v1/reversals';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GetReversalsResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GetReversalsResponse");
